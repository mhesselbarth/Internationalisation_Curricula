...
### Set-up
First, we load the [`spatstat`](https://spatstat.org) library for spatial point pattern analysis (Baddeley and Turner, 2005). Also, we import the [`tidyverse`](https://www.tidyverse.org) library, a collection of `R` packages designed for data science (Wickham, 2017). To reproduce the analysis we also need to load the [`raster`](https://cran.r-project.org/package=raster) (Hijmans, 2019) package and for an easy layout of result plots, the [`patchwork`](https://patchwork.data-imaginist.com) (Pedersen, 2019) package.
...
### Import the data
We use the [`readr`](https://readr.tidyverse.org/index.html) (Wickham et al., 2018) package to import the data because it will automatically be imported as a [`tibble`](https://tibble.tidyverse.org/index.html) (Müller and Wickham, 2019; an advanced `data.frame`). For more information see `?tibble`. But, of course you can use any way of importing the data you prefer. The deliminator of the data is a comma. The data is seperated into two files for the years 1994 and 1995.

The data includes a coloumn with the date of sampling as well as a coloumn which identifies the type of the point ("Elevation", "New", "Old", "Stake", "Trap") We transform the date coloumn `as.Date` and order the data by `Type` and `Date` in the same pipe.
...
Firstly, we simply visualize the two point patterns of the north grid in 1994 and 1995. For this, we use the [`ggplot2`](https://ggplot2.tidyverse.org) (Wickham, 2016) package. But again, you can also use `base plot` to plot the coordinates if you prefer. To also plot the study plot (or observation window), we create a `tibble` with the coordinates of the corner points in counter-clockwise order 
...
Next, we can build both plots and combine them into on plot using `patchwork`.
...
Now, we can use the moving window approach to calculate the intensity/rate per area and time. Therefore, we create a raster grid to later store the rate of production. 
...
To acutally count the number of mounds within each grid cell, we convert this `raster` to a `tibble` again to loop through all rows and by that through each raster cell. Within the loop, we create a small `owin` object with the size of 20 $m^{2}$ using the cell centre as the centre of the `owin`. We then count the number of mounds within the window using `inside.owin()` and divide the count by the amount of time and the area. 
...
### Pooled L-function
To analyse the L-function for the pooled data set (both years combined), we firstly clip the study plot by removing the area that was flooded. Because we don't know the exact locations of this area, we have to estimated it by try-and-error.
...
In the original publication, the center L-function was used. However, in `spatstat` only the "normal" L-function is available. Thus, we need to center it ourselves. The `eval.fv()` functions allows to evaluate any expression involving the result of a summary function in `spatstat` (`fv` object). Alternativly, you could also use the `center_l_function()` function from the [`onpoint`](https://r-spatialecology.github.io/onpoint/) (Hesselbarth, 2019)
...
To calculate the 5th highest and lowest values of the null model data and the observed function value, we use the `envelope()` function. If we do not specify a specifc null model, complete spatial randomness (CSR) is simulated automatically. 
